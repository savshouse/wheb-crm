...
export default function EditMeetingForm({ clientId, meeting, profiles, currentUserId, templates, existingTasks }: Props) {
  ...
  const [meetingDate, setMeetingDate] = useState(meeting.meeting_date.split('T')[0])
  ...
  function updateTaskField(id: string, field: keyof TaskDraft, value: string) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (validTasks.length > 0) {
      ...
      if (user) {
        const { data: created, error: tasksError } = await supabase.from('tasks').insert(
          validTasks.map(t => ({
            title:       t.title.trim(),
            client_id:   clientId,
        ...
        if (created) {
          await supabase.from('task_history').insert(
            created.map(task => ({
              ...
              note:         `Created from meeting: ${title}`,
            }))
          )
        }
      }
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
          ...
          <div>
            ...
            <input type="date" value={meetingDate} onChange={e => setMeetingDate(e.target.value)} required className={inputClass} />
          </div>
          <div>
        ...
        {existingTasks.length > 0 && (
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <h2 className="text-sm font-semibold text-slate-700 mb-3">
              Existing tasks from this meeting ({existingTasks.length})
            </h2>
                ...
                return (
                  <div key={task.id} className="flex items-center gap-3 p-2.5 rounded-lg border border-slate-100 bg-slate-50">
                    <div className="flex-1 min-w-0">
                      ...
                      {assignee && <p className="text-xs text-slate-400 mt-0.5">{assignee.full_name ?? assignee.email}</p>}
                    </div>
                    {task.due_date && <span className="text-xs text-slate-500 shrink-0">{task.due_date}</span>}
                    <span className={`shrink-0 text-xs px-1.5 py-0.5 rounded font-medium ${priorityColour[task.priority] ?? ''}`}>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-sm font-semibold text-slate-700">Add follow-up tasks</h2>
            ...
            <div className="flex items-center gap-2">
              {templates.length > 0 && (
                <div className="relative">
                  <button type="button" onClick={() => setShowTemplateMenu(p => !p)}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-purple-50 text-purple-700 text-sm font-medium hover:bg-purple-100 border border-purple-200">
              ...
              <button type="button" onClick={addTask}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-50 text-blue-700 text-sm font-medium hover:bg-blue-100 border border-blue-200">
                ...
                <div key={task.id} className="border border-slate-200 rounded-xl p-4 space-y-3 bg-slate-50">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div>
                      ...
                      <select value={task.assigned_to} onChange={e => updateTaskField(task.id, 'assigned_to', e.target.value)}
                    ...
                    <div>
                      ...
                      <input type="date" value={task.due_date} onChange={e => updateTaskField(task.id, 'due_date', e.target.value)}
                        className="w-full px-2 py-1.5 rounded-lg border border-slate-300 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" />
                    </div>
                    <div>
                      <label className="block text-xs text-slate-500 mb-1">Priority</label>
                      <select value={task.priority} onChange={e => updateTaskField(task.id, 'priority', e.target.value as 'low' | 'medium' | 'high')}
                        className="w-full px-2 py-1.5 rounded-lg border border-slate-300 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
        ...
        <div className="flex items-center gap-3">
          <button type="submit" disabled={saving}
            className="px-6 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 disabled:opacity-50">
            {saving ? 'Saving...' : validNewCount > 0
              ? `Save changes & ${validNewCount} task${validNewCount !== 1 ? 's' : ''}`
              : 'Save changes'
            }
          </button>
          <Link href={`/clients/${clientId}`}
            className="px-5 py-2.5 rounded-xl border border-slate-300 text-slate-600 text-sm font-medium hover:bg-slate-50">
            Cancel
          </Link>
          {validNewCount > 0 && (
            <span className="text-xs text-slate-500 ml-auto">
              {validNewCount} task{validNewCount !== 1 ? 's' : ''} will be created
            </span>
          )}
        </div>
      </form>
